...
export default {
    ...
    computed: {
        ...
        localeFilterDateRange() {
            if (this.filterDateRange === undefined) {
                return this.t("i.table.filterDateRange");
            } else {
                return this.filterDateRange;
            }
        },
        localeFilterDateRangePlaceHolder() {
            if (this.filterDateRangePlaceHolder === undefined) {
                return this.t("i.table.filterDateRangePlaceHolder");
            } else {
                return this.filterDateRangePlaceHolder;
            }
        },
    ...
    methods: {
            ...
            if (this.serverSide) {
                ...
                if (this.fetchMethod) {
                    this.fetchMethod(
                        this.getSortInfo(),
                        this.getFilterInfo(),
                        this.$refs.page.currentPage,
                        this.$refs.page.currentPageSize
                    );
                } else {
                    console.log("not define fetchMethod!");
                }
                return;
            }
        ...
        getSortInfo() {
            ...
            for (let i = 0; i < this.leftFixedFilterRows.length; i++) {
                if (this.leftFixedFilterRows[i].fixed === "left") {
                    if (
                        this.leftFixedFilterRows[i]._sortType === "desc" ||
                        this.leftFixedFilterRows[i]._sortType === "asc"
                    ) {
                        sortInfos.push({
                            field: this.leftFixedFilterRows[i].key,
                            sortType: this.leftFixedFilterRows[i]._sortType
                        });
                    }
                }
            }
            ...
            for (let i = 0; i < this.filterRows.length; i++) {
                if (
                    this.filterRows[i].fixed === "right" ||
                    this.filterRows[i].fixed === "left"
                ) {
                    continue;
                }
                if (
                    this.filterRows[i]._sortType === "desc" ||
                    this.filterRows[i]._sortType === "asc"
                ) {
                    sortInfos.push({
                        field: this.filterRows[i].key,
                        sortType: this.filterRows[i]._sortType
                    });
                }
            }
            ...
            for (let i = 0; i < this.rightFixedFilterRows.length; i++) {
                if (this.leftFixedFilterRows[i].fixed === "right") {
                    if (
                        ...
                        sortInfos.push({
                            field: this.rightFixedFilterRows[i].key,
                            sortType: this.rightFixedFilterRows[i]._sortType
                        });
                    }
                }
            }
        ...
        getFilterInfo() {
            ...
            for (let i = 0; i < this.leftFixedFilterRows.length; i++) {
                if (this.leftFixedFilterRows[i].fixed === "left") {
                    if (
                        this.leftFixedFilterRows[i]._filterValue !== null &&
                        this.leftFixedFilterRows[i]._filterValue !==
                            undefined &&
                        this.leftFixedFilterRows[i]._filterValue !== ""
                    ) {
                        filterInfos.push({
                            field: this.leftFixedFilterRows[i].key,
                            type: this.leftFixedFilterRows[i]._type,
                            operation: this.leftFixedFilterRows[i]._filterOp
                                .filterOperation,
                            value: this.leftFixedFilterRows[i]._filterValue
                        });
                    }
                }
            }

            this.filterRows.forEach(column => {
                if (
                    ...
                    filterInfos.push({
                        field: column.key,
                        type: column._type,
                        operation: column._filterOp.filterOperation,
                        value: column._filterValue
                    });
                } else {
                }
            });
            for (let i = 0; i < this.rightFixedFilterRows.length; i++) {
                if (this.rightFixedFilterRows[i].fixed === "right") {
                    if (
                        this.rightFixedFilterRows[i]._filterValue !== null &&
                        this.rightFixedFilterRows[i]._filterValue !==
                            undefined &&
                        this.rightFixedFilterRows[i]._filterValue !== ""
                    ) {
                        filterInfos.push({
                            field: this.rightFixedFilterRows[i].key,
                            type: this.rightFixedFilterRows[i]._type,
                            operation: this.rightFixedFilterRows[i]._filterOp
                                .filterOperation,
                            value: this.rightFixedFilterRows[i]._filterValue
                        });
                    }
                }
            }
        ...
        exportXlsxJson({ fileName, header, headerDisplay, rows, formats }) {
            ...
            if (rows !== undefined && rows !== null) {
                for (let m = 0; m < rows.length; m++) {
                    let row = {};
                    for (let n = 0; n < defaultHeader.length; n++) {
                        row[defaultHeader[n]] = rows[m][defaultHeader[n]];

                        if (formats !== null && formats !== undefined) {
                            if (
                                formats[defaultHeader[n]] !== undefined &&
                                formats[defaultHeader[n]] !== null &&
                                formats[defaultHeader[n]].constructor ===
                                    Function
                            ) {
                                try {
        ...
        data: {
            handler(nVal, oVal) {
                if (this.serverSide) {
                    setTimeout(() => {
                        this.objData = this.makeObjData();
                        this.rebuildDataStr = this.makeData();
