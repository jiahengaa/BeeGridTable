...
export declare class Table extends Vue {
    ...
    width?: number | string;
    /**
     * 表格高度，单位 px，当设置后，如果表格内容大于此值，会固定表头
     */
    height?: number | string;
    /**
     * 表格最大高度，单位 px，当设置后，如果表格内容大于此值，会固定表头
     */
     ...
     * column：当前列数据
     */
    $emit(eventName: "on-sort-change", column?: object): this;
    /**
...
export declare class TableColumn {
    ...
    /**
     * 自定义列头显示内容，传入参数有两个，column 和 index，分别为当前列数据和当前列索引
     * @param h Render函数
     * @param params 对象，包含 row、column 和 index
     */
    renderHeader?: (
